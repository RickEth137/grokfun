/**
 * simple-discriminator-fix.js - Simplest fix with the correct discriminator
...
async function main() {
  ...
  try {
    ...
    // Send a minimal amount of SOL to the vault to ensure it exists and is system-owned
    console.log("\nEnsuring vault SOL PDA exists as system-owned account...");
    const createVaultTx = new Transaction().add(
      SystemProgram.transfer({
        fromPubkey: wallet.publicKey,
        toPubkey: vaultSolPda,
        lamports: 10000, // Just a minimal amount
      })
    );
    
    const createVaultSig = await sendAndConfirmTransaction(
      connection,
      createVaultTx,
      [wallet]
    );
    console.log(`Created vault SOL PDA (or sent SOL if it already exists). Signature: ${createVaultSig}`);
    ...
    // Write parameters in the same order as in lib.rs
    let offset = 8;
