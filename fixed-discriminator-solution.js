/**
 * fixed-discriminator-solution.js - Using the correctly computed discriminator for initialize_launch
 */
const { 
  Connection, 
  PublicKey, 
...
async function main() {
  ...
  try {
    ...
    // Initialize launch parameters - similar to lib.rs function signature
    ...
    // Create initialization instruction with correct account structure from lib.rs
    const initIx = {
    ...
    if (error.signature) {
      try {
        console.log("\nAttempting to fetch detailed transaction logs...");
        setTimeout(async () => {
          try {
        ...
        console.log("Error fetching transaction details:", e.message);
      }
    }
